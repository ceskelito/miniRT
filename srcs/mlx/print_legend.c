...
/*   Updated: 2026/04/10 16:28:18 by rceschel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	if (rt->scene.expanded_legend != NO_LEGEND)
		height += CHAR_HEIGHT * count_elem(g_legends[rt->scene.expanded_legend]);
...
static void print_text(t_minirt *rt)
{
	...
	for (int i = 0; i < MENU_ITEMS; i++)
	{
		...
		mlx_string_put(rt->mlx.ptr, rt->mlx.win, X + 10, printing_height, 0xFFFFFF, (char *)g_legends[i][0]);
		if (rt->scene.expanded_legend == i)
		{
			for (int j = 1; g_legends[i][j]; j++)
			{
				printing_height += (CHAR_HEIGHT);
				mlx_string_put(rt->mlx.ptr, rt->mlx.win, X * 3 + 10, printing_height, 0xFFFFFF, (char *)(g_legends[i][j]));
			}
		}
